...
function GroupCreatePage(): JSX.Element {
  ...
  const requestGroupCreate = async () => {
    try {
      await groupClient.postGroupCreate(groupData);
  ...
  useEffect(() => {
    const fetchCategoryList = async () => {
      const response: Category[] = await categoryClient.getCategories();
      setCategorys(response);
    };

    const fetechTechStackList = async () => {
      const response: TechStack[] = await techStackClient.getTechStackList();
